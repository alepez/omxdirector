// Use a PTY to circumvent STDOUT buffering. This is what expect does. See
...
var spawn = require('pty.js').spawn
  , debug = require('debug')('omxdirector')
  , util = require('util')
  , path = require('path')
  , fs = require('fs')
  , EventEmitter = require('events').EventEmitter
  , LoopHelper = require('./loop_helper.js').LoopHelper
  , exec = require('child_process').exec
  , onlyOneProcess = require('./watchdog.js').onlyOneProcess;
...
var omxdirector = function () {
  ...
  var sendAction = function (action) {
    if (commands[action] && omxProcess) {
      try {
        omxProcess.stdin.write(commands[action], undefined, function() {
          debug("Sent command " + action + " -> " + commands[action]);
        });
      } catch (err) {
        console.log("Error sending " + action + " command: " + err);
      }
    }
  };
  ...
  var readSTDOUTLine = function(line) {
    var match = /M:\s*(\d+)/.exec(line);
    if (match) {
      var status = {
        seconds: parseInt(match[1], 10) / 1000000
      };
  ...
  var open = function (videos, options) {
    ...
    debug( "Starting omxplayer with " + cmd + " " + args.join(" ") );
    ...
    if (!nativeLoop && ((realfiles.length > 1) || (settings.loop))) {
      ...
      respawn = function () {
        ...
        if (!nextFile) {
          ...
          omxProcess = spawn(cmd, args, { stdio: stdio });
    ...
    var buffer = '';
    omxProcess.stdout.on('data', function(d) {
  ...
  ['volup', 'voldown', 'faster', 'slower', 'step', 'chapternext', 'chapterprev'].forEach(function(c) {
    that[c] = function() {
      sendAction(c);
      that.emit(c);
      return true;
