...
var omxdirector = function () {
    ...
    if (commands[action] && omxProcess) {
      try {
        omxProcess.stdin.write(commands[action], undefined, function() {
          console.log("Sent command", action, commands[action]);
        });
      } catch (err) {
        console.log("Error sending", action, "command to child process:", err);
      }
    }
  };
  ...
  var open = function (videos, options) {
    ...
    var cmd = 'omxplayer';
    var stdio = options.debug ? 'pipe' : ['pipe', null, null];
    ...
    if (options.debug) {
      omxProcess.stdout.on("data", function(d) { console.log( "OMX OUT: "+d.toString() ); });
      omxProcess.stderr.on("data", function(d) { console.log( "OMX ERR: "+d.toString() ); });
    }
